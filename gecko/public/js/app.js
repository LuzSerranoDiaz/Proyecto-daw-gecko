...
$('.filter').change(function() {
    if(this.checked) {
        let showOnlyNotSolved = $('.filter#solved')[0].checked === false ? 1 : 0;
        switch (this.id){
            case 'green':
                showOnlyNotSolved ? $('.color-3').parent().parent().parent().find('.not-solved').removeClass('hidden') :
                $('.color-3').parent().parent().removeClass('hidden')
                break;
            case 'yellow':
                showOnlyNotSolved ? $('.color-2').parent().parent().parent().find('.not-solved').removeClass('hidden') :
                $('.color-2').parent().parent().removeClass('hidden')  
                break;
            case 'red':
                showOnlyNotSolved ? $('.color-1').parent().parent().parent().find('.not-solved').removeClass('hidden') :
                $('.color-1').parent().parent().removeClass('hidden')
                break;
            case 'solved':
                $('.not-solved').removeClass('hidden');
                $('.fa-check').parent().parent().removeClass('hidden')
                break;
        }
    } else {
        switch (this.id){
            case 'green':
                $('.color-3').parent().parent().addClass('hidden')
                break;
            case 'yellow':
                $('.color-2').parent().parent().addClass('hidden') 
                break;
            case 'red': 
                $('.color-1').parent().parent().addClass('hidden')
                break;
            case 'solved':
                $('.fa-check').parent().parent().addClass('hidden')
                break;
        }
    }
})
...
$('body').on('click', '.addComment', function(e){
    $(this).closest('.modal-body').find('form').removeClass('hidden');
})
